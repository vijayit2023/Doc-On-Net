...
def verify():
    ...
    mail.send(msg)
    flash("Verification mail has been successfully sent to your email","info")
    return redirect(url_for("home"))
...
def dashboard():
    ...
    cur.execute("SELECT * FROM (SELECT * FROM user ORDER BY pid DESC LIMIT 10 ) ORDER BY pid ASC")
    user=cur.fetchall()
...
def visual():
    ...
    cur.execute("select BMI from details where pid=(SELECT COUNT(Age) FROM details)")

    total = cur.fetchone()

    value = total['BMI']

    return render_template('visual.html',Name=session["name"],Email=session["email"], value=value)
